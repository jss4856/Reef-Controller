...
int blueramptime = 20 ;    // time for blue LEDs to dim on and off in minutes ***THIS is multiplied to be longer (somehow?) by temperature(); and topoff();
...
int photoperiod = 615 ;    // amount of time array is on at full power in minutes
int ontime = 9 ;          // time of day (hour, 24h clock) to begin photoperiod fade in
int blue = 5;              // blue LEDs connected to digital pin 3 (pwm)
int white = 3;            // white LEDs connected to digital pin 5 (pwm)
int lights = 8;        //this fan is for cooling the LEDs ***RETIRED

int bluepercent[11] = { 
  0, 26, 50, 75, 100 , 125, 150, 175, 200, 225, 255 };   // this line is needed if you are using meanwell ELN60-48P
int whitepercent[11] = { 
  0, 26, 50, 75, 100 , 125, 150, 175, 200, 225, 255 };   // these are the values in ~10% increments does not responde to <28
  

/*|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||  C L O U D S  |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*/
/*||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*/
//defines led random intensity range
int randNumber = random(35, 150);
...
void topoff()
{
  ...
  if (floatstate == HIGH){
    digitalWrite(topoffpump, LOW);//adding water
  }
  else{
    digitalWrite(topoffpump,HIGH);//not adding water
  }
  delay (1000);
}
...
void loop()
{
  ...
  { 
    if ( daybyminute < ((ontime * 60) + blueramptime + whiteramptime + photoperiod)) // if time is in range of photoperiod, turn lights on to maximum fade value
    {
      ...
      /*
      for(int fade1Value = bluemax ; fade1Value >= randNumber; fade1Value -=5)
      { 
        lcd.setCursor(0, 2);
      ...
      for(int fade2Value = randNumber; fade2Value <= bluemax; fade2Value +=5) 
      { 
        lcd.setCursor(0, 2);
        lcd.print("Sunshine        ");
